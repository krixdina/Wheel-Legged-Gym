...
def parse_args():
    ...
    p.add_argument("--height", type=float, default=C.DEFAULT_HEIGHT, help="body height command [m]")
...
def reset_state(data):
    ...
    data.qpos[:] = 0.0
    data.qvel[:] = 0.0
    data.qpos[2] = 0.35           # base z (init_state.pos)
    data.qpos[3:7] = [1, 0, 0, 0]  # identity quaternion [w,x,y,z]
    data.qpos[QPOS_JOINT_START:] = C.DEFAULT_DOF_POS


def main():
    args = parse_args()
    model = mujoco.MjModel.from_xml_path(MODEL_XML)
    model.opt.timestep = C.SIM_DT
    ...
    last_actions = np.zeros(C.NUM_ACTIONS, dtype=np.float32)
    ...
    def read_observation():
        ...
        return np.clip(obs, -C.CLIP_OBSERVATIONS, C.CLIP_OBSERVATIONS), legs, dof_vel

    obs, legs, dof_vel = read_observation()
    obs_history = np.tile(obs, C.OBS_HISTORY_LENGTH)

    n_policy_steps = int(args.seconds / (C.SIM_DT * C.DECIMATION))
    ...
    def run_loop(viewer=None):
        ...
        for step in range(n_policy_steps):
            ...
            action = np.clip(action, -C.CLIP_ACTIONS, C.CLIP_ACTIONS)
            ...
            obs_history = np.concatenate([obs_history[C.NUM_OBS:], obs])
            ...
            if viewer is not None:
                ...
                dt = C.SIM_DT * C.DECIMATION - (time.time() - t0)
                if dt > 0:
                    time.sleep(dt)
            elif step % 100 == 0:
